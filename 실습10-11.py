...
class CNN(torch.nn.Module):
    ...
    def __init__(self, ch1, ch2):
        ...
        self.layer2 = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(kernel_size=2, stride=2))
        self.fc1 = torch.nn.Linear(7*7*ch2, 200, bias=True)
        self.fc2 = torch.nn.Linear(200,10, bias=True)
...
for epoch in range(max_epoch):
    ...
    for inp, ans in data_loader:
        ...
        with torch.no_grad():
            X_test = mnistTest.data.view(len(mnistTest), 1, 28, 28).float().to(device)[0:1000]
            Y_test = mnistTest.targets.to(device)[0:1000]
            y_t = model(X_test)
